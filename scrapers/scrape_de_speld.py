...
Script retrieves news article URLs from De Speld sitemap and scrapes all news articles from last 5 years

De Speld sitemap index found from robots.txt: https://speld.nl/robots.txt
...
# Constants
SITEMAP_INDEX_URL = "https://speld.nl/sitemap_index.xml"
HEADERS = {
...
def get_sitemap_files(soup):
    """Function retrieves XML files from DeSpeld sitemap
    Args:
...
def get_news_article_urls(xml_path: str):
    """Function retrieves news article URLs from DeSpeld sitemap XMLs
    Args:
...
def get_news_article_data(article_url: str):
    ...
    # Ignore articles where title is section header
    if title and (title == "De Speld Sport" or title == "De Pin"):
        return None
    ...
    if document_text:
        ...
        # Clean text beginning if keyword found
        if (
            any(
                word in text
                for word in [
...
if __name__ == "__main__":
    ...
    for sitemap_path in found_sitemap_files[1:]:
        ...
        df.to_csv(
            f"./{CURRENT_DIRECTORY}/de_speld/de_speld_scraped_articles_{current_year}{current_month_str}{current_day_str}_filenumber_{file_num}.csv",
            index=False,
        )
        file_num += 1
